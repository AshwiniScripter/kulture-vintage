                ...
                cartItems.map((item) => (
                  <div key={item.id} className="flex gap-4 p-3 bg-[#111111] rounded-xl border border-neutral-900">
                    {item.image ? (
                      ...
                      <div className="w-20 h-20 rounded-lg shrink-0 border border-neutral-800 bg-neutral-800/40 flex items-center justify-center">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between">
                      <div>
                        ...
                        <p className="text-[10px] text-neutral-500 tracking-wide mt-0.5">{item.color || 'Black'} . {item.size || 'M'}</p>
                        <p className="text-sm font-bold mt-1 text-white">{item.priceDisplay || (typeof item.price === 'number' ? `₹${item.price.toLocaleString('en-IN')}` : item.price)}</p>
                      </div>
                ...
                <div className="bg-[#0f0f0f] border border-neutral-900 rounded-xl p-3.5 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-[10px] text-neutral-500 tracking-wider">
                      {appliedDiscount > 0 ? 'DISCOUNT APPLIED' : 'TAXES INCLUDED'}
                    </div>
                  </div>
